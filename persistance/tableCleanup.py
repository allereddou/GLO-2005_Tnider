...
def createTables(cursor):
    ...
    sql = "CREATE TABLE IF NOT EXISTS preferencesBird(username VARCHAR(20), blackBirb TINYINT, whiteBirb TINYINT, blueBirb TINYINT, beigeBirb TINYINT, greyBirb TINYINT, greenBirb TINYINT, yellowBirb TINYINT, 0_5AgeBirb TINYINT, 5_10AgeBirb TINYINT, 10AgePlusBirb TINYINT, 0_1WeightBirb TINYINT, 1_2WeightBirb TINYINT, 2PlusWeightBirb TINYINT, femaleBirb TINYINT, maleBirb TINYINT, FOREIGN KEY(username) REFERENCES user(username) ON DELETE CASCADE ON UPDATE CASCADE, PRIMARY KEY(username));"
    cursor.execute(sql)

    sql = "CREATE TABLE IF NOT EXISTS preferencesCat(username VARCHAR(20), declawedCat TINYINT, whiteCat TINYINT, blackCat TINYINT, gingerCat TINYINT, greyCat TINYINT, brownCat TINYINT, castratedCat TINYINT,femaleGenderCat TINYINT, maleGenderCat TINYINT, 0_10WeightCat TINYINT, 10_20WeightCat TINYINT, 20PlusWeightCat TINYINT, 0_5AgeCat TINYINT, 5_10AgeCat TINYINT, 10PlusAgeCat TINYINT, FOREIGN KEY(username) REFERENCES user(username) ON DELETE CASCADE ON UPDATE CASCADE, PRIMARY KEY(username));"
    cursor.execute(sql)
